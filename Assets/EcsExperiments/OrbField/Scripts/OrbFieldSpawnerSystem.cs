            ...
            .ForEach((Entity entity, int entityInQueryIndex, in OrbFieldSpawner spawner, in LocalToWorld location) =>
            {
                ...
                for (var x = 0; x < spawner.CountX; x++)
                {
                    for (var y = 0; y < spawner.CountY; y++)
                    {
                        ...
                        //     new LocationComponent() { InitialPosition = translation });
                        ...
                        // commandBuffer.SetComponent(entityInQueryIndex, instance, new RotationSpeed_EyeLasers { RadiansPerSecond = math.radians(random.NextFloat(25.0F, 90.0F)) });
