...
namespace Samples.Boids
{
    ...
    public class BoidSystem : JobComponentSystem
    {
        ...
        struct MergeCells : IJobNativeMultiHashMapMergedSharedKeyIndices
        {
            ...
            void NearestPosition(NativeArray<float3> targets, float3 position, out int nearestPositionIndex,
        ...
        protected override JobHandle OnUpdate(JobHandle inputDeps)
        {
            ...
            for (int boidVariantIndex = 0; boidVariantIndex < m_UniqueTypes.Count; boidVariantIndex++)
            {
                ...
                var cellObstaclePositionIndex = new NativeArray<int>(boidCount, Allocator.TempJob,
                ...
                var cellObstacleDistance =
                    new NativeArray<float>(boidCount, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
                ...
                var initialCellBarrierJobHandle = JobHandle.CombineDependencies(initialCellAlignmentJobHandle,
                    initialCellSeparationJobHandle, initialCellCountJobHandle);
                var copyTargetObstacleBarrierJobHandle =
                    JobHandle.CombineDependencies(copyTargetPositionsJobHandle, copyObstaclePositionsJobHandle);
                var mergeCellsBarrierJobHandle = JobHandle.CombineDependencies(hashPositionsJobHandle,
                    initialCellBarrierJobHandle, copyTargetObstacleBarrierJobHandle);
                ...
                var steerJobHandle = Entities
                        ...
                        ref ColorComponent colorComponent, ref EmissionComponent emissionComponent) =>
                    {
                        ...
                        var nearestObstacleDistanceFromRadius =
                            nearestObstacleDistance - settings.ObstacleAversionDistance;
                ...
                disposeJobHandle =
                    JobHandle.CombineDependencies(disposeJobHandle, cellObstaclePositionIndex.Dispose(inputDeps));
                disposeJobHandle =
                    JobHandle.CombineDependencies(disposeJobHandle, cellTargetPositionIndex.Dispose(inputDeps));
                disposeJobHandle = JobHandle.CombineDependencies(disposeJobHandle, cellCount.Dispose(inputDeps));
                disposeJobHandle =
                    JobHandle.CombineDependencies(disposeJobHandle, cellObstacleDistance.Dispose(inputDeps));
                ...
                disposeJobHandle =
                    JobHandle.CombineDependencies(disposeJobHandle, copyObstaclePositions.Dispose(inputDeps));
                disposeJobHandle =
                    JobHandle.CombineDependencies(disposeJobHandle, copyTargetPositions.Dispose(inputDeps));
